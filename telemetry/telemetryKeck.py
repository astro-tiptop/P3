...
class telemetryKeck:
    ...
    def restoringInstrumentData(self,cam_fov=None):
        ...
        if self.tel.pupilMaskName.upper() == 'LARGEHEX':
            ...
            self.tel.path_pupil = self.path_calib + '/NIRC2_MASK/keck_pupil_open_240px.fits'
        ...
        if self.cam.name == 'OSIRIS':
            ...
            self.cam.wvl, self.cam.bw, self.cam.transmission, self.cam.dispersion =\
            keckUtils.samplingFilterProfile(self.path_filter, hdr)
    ...
    def restoringCalibrationData(self, nLayer=1):
        ...
        if self.path_trs is not None:
            dm = deformableMirror(self.dm.nActuators,
                                  self.dm.pitch,
                                  heights=self.dm.heights,
                                  mechCoupling=self.dm.mechCoupling,
                                  modes=self.dm.modes)
        ...
        if nLayer < len(self.atm.Cn2Heights):
            self.atm.wSpeed = list(FourierUtils.eqLayers(np.array(self.atm.Cn2Weights),
                                                         np.array(self.atm.wSpeed),
                                                         nLayer)[1])
            self.atm.wDir = list(FourierUtils.eqLayers(np.array(self.atm.Cn2Weights),
                                                       np.array(self.atm.wDir),
                                                       nLayer,power=1)[1])
            self.atm.Cn2Weights, self.atm.Cn2Heights = FourierUtils.eqLayers(self.atm.Cn2Weights,
