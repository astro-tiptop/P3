...
def makeKeckPupil():
    path_txt = os.getcwd() + '/_txtFile/Keck_segmentVertices.txt'
...
def makeELTPupil():
    path_txt = os.getcwd() + '/_txtFile/ELT_segmentVertices.txt'
...
#%% MANAGE REFLEXIVITY AND ABERRATIONS
    ...
    def computeModes(self,jIndex,area = 'segment'):
        ...
        if len(jIndex) == 0:
            print('Error : you musr provide a valist list of Noll index')
            return []
...
#%% RESIZE
    ...
    def resize(self,newNPixels,flagReplace=0,kind='nearest'):
        ...
        if newNPixels != self.nPixels:
            self.matrix = interpolateSupport(self.matrix,newNPixels,kind=kind)
