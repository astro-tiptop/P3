...
def generatePSF(path_ini,nIntervals=10,nPSFperFolder=3500,addStatic=False,mag=0,zP=25.44,DIT=0.5,nDIT=50,skyMag=13.6,ron=0,\
                normType=1,rootPath=None,nRound=7):
    ...
    if rootPath != None:
        ...
        if mag != 0:
            ...
            skyFlux = 10 ** (-0.4*(skyMag - zP)) * nDIT * DIT * (psfao.psInMas/1e3) ** 2
            ronStack= ron * np.sqrt(nDIT)
            idNOISE = ''      
        # defining prefix   
        instru    = path_ini.split('/')[-1].split('.')[0]
        psfPrefix = 'psfao21_' + instru + '_wvl_' + str(round(psfao.wvlRef*1e6,3)) + 'µm'
        # creating tha main data folder
        savePath = rootPath + '/PSFAO21_' + idNOISE + 'NOISE_' + idSTATIC + 'STATIC/'
        if not os.path.isdir(savePath):
            os.mkdir(savePath)
        
    # ------------------- DEFINING R0 AND A DISTRIBUTIONS OVER THE WHOLE DATA SETS
    # bounds
    r0_lb = 0.05*(wvl/500e-9)**1.2
    r0_ub = 0.3*(wvl/500e-9)**1.2
    A_lb  = (wvlFact*80)**2
    A_ub  = (wvlFact*390)**2
    # intervals
    ...
    for k in range(nIntervals):
        for j in range(nIntervals):
            ...
            if rootPath != None:
                ...
                idsub = 'r0_' + str(r0_int[k][0]) + '_sig2_' + str(A_int[j][0])
    ...
    C    = np.repeat(uniform(low=1e-4,high=5e-2,size=nPSFperFolder)[:,np.newaxis],nSubFolder,axis=1)
    ax   = np.repeat(uniform(low=1e-4,high=5e-2,size=nPSFperFolder)[:,np.newaxis],nSubFolder,axis=1)
    p    = np.repeat(np.linspace(0.5,2,nPSFperFolder)[:,np.newaxis],nSubFolder,axis=1)
    be   = np.repeat(uniform(low=1.1,high=3.0,size=nPSFperFolder)[:,np.newaxis],nSubFolder,axis=1)
    if addStatic:
        stat = np.repeat(uniform(low=-wvl*1e9/2,high=wvl*1e9/2,size=(6,nPSFperFolder))[:,:,np.newaxis],nSubFolder,axis=2)

    #%% LOOPS
    pix2freq = 1/(psfao.tel.D * psfao.sampRef)**2
    otfDL    = fft.fftshift(psfao.otfDL)
    idStat   = ''
    idNoise  = ''
    # PRE-CALCULATE PSD
    for j in range(nPSFperFolder):
        # DERIVING THE AO-CORRECTED PSD
        psdIn = psfao.pistonFilter_ * psfao.moffat(psfao.kxky_,[ax[j,0],p[j,0],0,be[j,0],0,0])
        psdIn = psdIn/psdIn.sum() / pix2freq
        # DERIVING THE TELESCOPE OTF
        if addStatic:
            otfStat = fft.fftshift(psfao.getStaticOTF(psfao.nOtf,psfao.sampRef,psfao.tel.obsRatio,psfao.wvlRef,xStat=stat[:,j,0]))
        else:
            otfStat = otfDL
                
        for k in range(nSubFolder):                        
            # Total PSD
            psd = r053[j,k] * (psfao.psdKolmo_) + psfao.mskIn_ * (C[j,0] + psdIn * A[j,k] )        
            # COMPUTING THE PSF
            ...
            # NORMALIZING PSF
            psf_i = normalizePSF(psf_i,normType=normType)    
            # SAVING
            if rootPath != None:
                # psf name
                if addStatic == True:
                ...
                idsub = 'r0_' + str(r0_int[idr0[j,k]][0]) + '_sig2_' + str(A_int[idA[j,k]][0])
                # save
